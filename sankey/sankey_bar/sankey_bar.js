...
function plotAxis(xMap, yMap, yOffset, xLab, yLab) {
	...
	var xAxisGrid = d3.axisTop(xMap)
		.ticks(3)
		.tickSize(yOffset - marginBar.top)
		.tickFormat("")
...
function plotTitle(selection , group) {
	d3.select("#title").style("marginBar-left", marginBar.left + "px");
...
function updateBarChart(selection, group, from=null, to=null) {
	...
	if (grouped != null) {
		...
		var TOP_BARS = 5
		var total_height = barHeight - (marginBar.top + marginBar.bottom);
		...
		if (grouped.length <= TOP_BARS) {
			total_height = bar_height * grouped.length +
				padding_inner * (grouped.length - 1) +
				2 * padding_outer;
		} else {
			total_height = bar_height * (TOP_BARS + 1)
				padding_inner * TOP_BARS +
				2 * padding_outer;
		}
		...
		var yMap = d3.scaleBand()
			.domain(names)
			.rangeRound([marginBar.top, total_height])
			.padding(0.4)
			.paddingOuter(0.1);
