...
function mapHeader(key) {
	switch (key) {
		case 'page_group':	return 'Page Group';
...
d3.csv('data/test.csv', 
	function(d) {
		return {
			page_group: d['Page Group'],
			users:
				{
					value: +parseFloatAnySeparator(d['Users']),
					change: d['Change in Users']
				},
			sessions:
				{
					value: +parseFloatAnySeparator(d['Sessions']),
					change: d['Change in Sessions']
				},
			pageviews:
				{
					value: +parseFloatAnySeparator(d['Pageviews']),
					change: d['Change in Pageviews']
				}
		};
	},
	function(e, d) {
